...
/* harmony export */   "lV": () => (/* binding */ addItemToWishList),
/* harmony export */   "Ok": () => (/* binding */ getUserWishLists),
/* harmony export */   "qn": () => (/* binding */ deleteWishLists),
/* harmony export */   "gU": () => (/* binding */ UpdateWishListQTY),
/* harmony export */   "PT": () => (/* binding */ deleteWishListItem)
/* harmony export */ });
/* unused harmony export showListItems */
const addItemToWishList = async (e, product, setFakeState)=>{
    e.preventDefault();
    let { URL  } = process.env;
    let listName = "";
    let itemQTY = "";
...
const deleteWishLists = async (userID, listName, setUserLists)=>{
    let { URL  } = process.env;
...
const showListItems = async (userID, listName)=>{
    let { URL  } = process.env;
    const lists = await fetch(URL + "/api/wishlist/deleteWishlists", {
        method: "POST",
        body: JSON.stringify({
...
const UpdateWishListQTY = async (e, action, id, setUserLists, userLists, userID, listName)=>{
    e.preventDefault();
    let { URL  } = process.env;
    let itemQty = 0;
    try {
    ...
    let index = -1;
    try {
        index = userLists.findIndex((list)=>list.userID === userID && list.name == listName
        );
    } catch (err1) {}
    if (index > -1) {
...
const deleteWishListItem = async (e, id, setUserLists, userLists, userID, listName, setWishListChanged, setListName)=>{
    e.preventDefault();
    let { URL  } = process.env;
    let index = -1;
    try {
        index = userLists.findIndex((list)=>list.userID === userID && list.name.toLowerCase() == listName.toLowerCase()
        );
    } catch (err) {}
